defmodule HelloWeb.ChatJoinLive do
  ...
  def render(assigns) do
    ~H"""
    <main
      class="max-w-lg mx-auto p-8 md:p-12 my-10 rounded-lg shadow-2xl bg-zinc-900"
    >
      <section>
      ...
      <section class="mt-10 ">
        <.form for={@form} phx-submit="save" class="flex flex-col ">
          ...
          <div class=" pt-3 rounded bg-zinc-900">
            <label
              class="block text-zinc-200 text-sm font-bold mb-2 ml-3"
              for="name"
              >Name</label>
            <.input
              field={@form[:name]}
              type="text"
              phx-change="search_room"
              id="name"
              required
            />
            <%= if @matches !=[]do %>
            <ul class="max-h-48 w-full my-3 p-2 overflow-y-scroll" id="rooms_list">
                <%= for room <- @matches do%>
                  <li 
                    class="my-1 p-4 border-2 bg-zinc-500 rounded-md border-zinc-800 relative cursor-pointer hover:text-gray-900"
                  >
                  <button 
          ...
          <.input type="password" field={@form[:pass]}
           class="bg-gray-500 rounded w-full text-gray-700 focus:outline-none border-none px-3 pb-3"/> --%>
          <button phx-disable-with="Creating account..." class="w-full bg-gray-500">Create a room</button>
        </.form>
      </section>
    </main>
    """
  ...
  def handle_event("save", %{"room"=>%{"name"=>name, "pass"=>passwd}}, socket) do
    ...
    if room==nil do
      ...
      if room.pass==passwd do # todo: change this
